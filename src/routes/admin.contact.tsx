...
import { CONTACT_KEY, CONTACT_DEFAULTS, type ContactOverride } from "@/lib/use-site-overrides";
import { toast } from "sonner";
import { Save, RotateCcw } from "lucide-react";

export const Route = createFileRoute("/admin/contact")({ component: AdminContactPage });
...
function AdminContactPage() {
  ...
  const [v, setV] = useState<ContactOverride>(CONTACT_DEFAULTS);
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    get({ data: { key: CONTACT_KEY } }).then((row) => {
      setV({ ...CONTACT_DEFAULTS, ...((row?.data as Partial<ContactOverride>) ?? {}) });
    });
  }, []);

  async function onSave() {
    setBusy(true);
    try { await save({ data: { key: CONTACT_KEY, data: v as unknown as Record<string, unknown> } }); qc.invalidateQueries({ queryKey: ["site-overrides"] }); toast.success("Saved"); }
    catch (e) { toast.error(e instanceof Error ? e.message : "Failed"); } finally { setBusy(false); }
  }
  async function onReset() {
    setBusy(true);
    try { await del({ data: { key: CONTACT_KEY } }); setV(CONTACT_DEFAULTS); qc.invalidateQueries({ queryKey: ["site-overrides"] }); toast.success("Reset"); }
    catch (e) { toast.error(e instanceof Error ? e.message : "Failed"); } finally { setBusy(false); }
  }
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Edit the hero of /contact. Phone / email / hours live under Site Chrome.</p>
        </div>
